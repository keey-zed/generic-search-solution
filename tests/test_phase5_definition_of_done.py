...
Phase 5's stated Definition of Done:
    ...
    "adding/removing/reordering a filter in the pilot's YAML changes the
...
here: mutating ONLY the pilot's config data (loaded, mutated as a plain
dict, and re-serialized -- the real `app/custom/legal_pilot/config.yaml`
...
`app/custom/legal_pilot/bootstrap.py`. Since those two endpoints are
...
`tests/test_pilot_definition_of_done.py` / `tests/test_http_api.py` --
...
_ORIGINAL_CONFIG_PATH = Path("app/custom/legal_pilot/config.yaml")


def _load_baseline_config_dict() -> dict[str, Any]:
    return yaml.safe_load(_ORIGINAL_CONFIG_PATH.read_text())


def _engine_from_config_dict(config: dict[str, Any]):
    """Serialize `config` to a temp YAML file and build a real
    SearchEngine from it -- reusing legal_pilot's own raw_loader /
...
# ---------------------------------------------------------------------------
# Baseline: the real, unmodified pilot config -- establishes "before" for
# the mutation tests below.
# ---------------------------------------------------------------------------


def test_baseline_pilot_config_exposes_six_filters_in_declared_order(baseline_config):
...
def test_adding_a_filter_appears_in_api_config_with_zero_code_changes(baseline_config):
    ...
    assert names[-1] == "cross_reference_count"  # respects the declared order: 7
...
def test_added_filter_also_appears_in_facets_with_zero_code_changes(baseline_config):
    ...
    mutated["frontend"]["filters"]["page_count"] = {"label": "Nombre de pages", "order": 7}
...
def test_removing_a_filter_disappears_from_api_config_with_zero_code_changes(baseline_config):
    mutated = copy.deepcopy(baseline_config)
    del mutated["filters"]["legal_status"]
    del mutated["frontend"]["filters"]["legal_status"]
    mutated["frontend"]["result_card_fields"] = [
        f for f in mutated["frontend"]["result_card_fields"] if f != "legal_status"
    ]
    ...
    body = client.get("/api/config").get_json()
    names = [f["name"] for f in body["filters"]]
    assert "legal_status" not in names
    assert len(names) == 5
    assert "legal_status" not in body["result_card_fields"]


def test_removed_filter_disappears_from_facets_with_zero_code_changes(baseline_config):
    mutated = copy.deepcopy(baseline_config)
    del mutated["filters"]["issuing_authority"]
    del mutated["frontend"]["filters"]["issuing_authority"]

    engine = _engine_from_config_dict(mutated)
    client = _client_for(engine)
    facets = client.get("/api/facets").get_json()["filters"]
    assert "issuing_authority" not in facets
...
def test_reordering_filters_changes_api_config_order_with_zero_code_changes(baseline_config):
    mutated = copy.deepcopy(baseline_config)
    mutated["frontend"]["filters"]["publication_date"]["order"] = 2
    mutated["frontend"]["filters"]["promulgation_date"]["order"] = 1

    engine = _engine_from_config_dict(mutated)
    client = _client_for(engine)
    body = client.get("/api/config").get_json()
    names = [f["name"] for f in body["filters"]]
    # promulgation_date (order: 1) now comes before publication_date (order: 2) --
    # the exact opposite of the baseline order asserted above.
    assert names[0] == "promulgation_date"
    assert names[1] == "publication_date"
    assert names[2:] == ["document_type", "issuing_authority", "legal_status", "title"]
...
def test_changing_a_control_changes_api_config_with_zero_code_changes(baseline_config):
    mutated = copy.deepcopy(baseline_config)
    assert mutated["frontend"]["filters"]["issuing_authority"]["control"] == "dropdown"
    mutated["frontend"]["filters"]["issuing_authority"]["control"] = "radio"

    engine = _engine_from_config_dict(mutated)
    client = _client_for(engine)
    body = client.get("/api/config").get_json()
    field = next(f for f in body["filters"] if f["name"] == "issuing_authority")
